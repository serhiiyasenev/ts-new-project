...
describe("Posts API (comprehensive)", () => {
  ...
  async function createUser() {
    const res = await request(app)
      .post("/users")
      .send({ name: "PostUser", email: `postuser${Date.now()}@example.com` })
      .expect(201);
    return res.body;
  }
  ...
  it("rejects invalid POST userId", async () => {
    const res = await request(app)
      .post("/posts")
      .send({ title: "x", content: "y", userId: 0 });
  ...
  it("requires userId when creating a post", async () => {
    await request(app)
      .post("/posts")
      .send({ title: "MissingUser", content: "Body" })
      .expect(400);
  });
  ...
  it("rejects POST when referenced user does not exist", async () => {
    const res = await request(app)
      .post("/posts")
      .send({ title: "NoUser", content: "X", userId: 9999 });
  ...
  it("creates a post (POST) and returns 201 with included user", async () => {
    ...
    const res = await request(app)
      .post("/posts")
      .send({ title: "Hello", content: "World", userId: user.id })
      .expect(201);
  ...
  it("validates POST body and returns 400 when missing required fields", async () => {
    ...
    await request(app)
      .post("/posts")
      .send({ content: "no title", userId: user.id })
      .expect(400);
    await request(app).delete(`/users/${user.id}`).expect(204);
  });
  ...
  it("GET /posts supports filters: title, content, userId", async () => {
    ...
    const { body: created } = await request(app)
      .post("/posts")
      .send({ title, content: "filtercontent", userId: user.id })
      .expect(201);
  ...
  it("CRUD for posts: create -> get -> update -> delete and validation on update", async () => {
    ...
    const { body: created } = await request(app)
      .post("/posts")
      .send({ title: "CrudPost", content: "c", userId: user.id })
      .expect(201);

    await request(app).get(`/posts/${created.id}`).expect(200);

    await request(app)
      .put(`/posts/${created.id}`)
      .send({ actorUserId: user.id, title: "" })
      .expect(400);
    await request(app)
      .put(`/posts/${created.id}`)
      .send({ actorUserId: user.id })
      .expect(400);

    // valid partial update
    const updated = await request(app)
      .put(`/posts/${created.id}`)
      .send({ actorUserId: user.id, content: "updated" })
      .expect(200);
  ...
  it("prevents updating a post by a different user and requires actorUserId", async () => {
    ...
    const { body: post } = await request(app)
      .post("/posts")
      .send({ title: "Secure", content: "Secret", userId: owner.id })
      .expect(201);
